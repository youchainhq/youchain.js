...
/**
 * Hex encodes the data passed to you_sign and personal_sign
 *
...
/**
 * TODO: Check where this is used and why the method below (outputPoolTransactionFormatter) also exists.
 * Formats the output of a transaction to its proper values
 *
 * @method outputPoolTransactionFormatter
 *
 * @param {Object} receipt
 *
 * @returns {Object}
 */
export const outputPoolTransactionFormatter = (receipt) => {
    if (receipt.gasPrice) {
        receipt.gasPrice = outputBigNumberFormatter(receipt.gasPrice);
    }
...
/**
 * TODO: Check where this is used and why the method below (inputCreateValidatorFormatter) also exists.
 * Formats the output of a validator to its proper values
 *
 * @method inputCreateValidatorFormatter
 *
 * @param {Object} options
 *
 * @returns {Object}
 */
export const inputCreateValidatorFormatter = (options) => {
    if (options.operator) {
        options.operator = inputAddressFormatter(options.operator);
    }

    if (options.coinbase) {
        options.coinbase = inputAddressFormatter(options.coinbase);
    }

    if (Utils.isHexStrict(options.nonce)){
        options.nonce = Utils.hexToNumber(options.nonce);
    }
...
/**
 * TODO: Check where this is used and why the method below (inputBroadcastCreateValidatorFormatter) also exists.
 * Formats the output of a transaction and a validator to its proper values
 *
 * @method inputBroadcastCreateValidatorFormatter
 *
 * @param {Object} options
 *
 * @returns {Object}
 */
export const inputBroadcastCreateValidatorFormatter = (options, moduleInstance) => {
    if (options.operator) {
        options.operator = inputAddressFormatter(options.operator);
    }
