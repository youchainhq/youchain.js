...
export default class Accounts extends AbstractYOUChainModule {
    /**
    ...
    signTransaction(tx, privateKey, callback) {
        ...
        function signed(tx) {
            ...
            if (tx.nonce < 0 || tx.gas < 0 || tx.gasPrice < 0 || tx.networkId < 0) {
                error = new Error('Gas, gasPrice, nonce or networkId is lower than 0');
            }
            ...
            try {
                ...
                transaction.networkId = _this.utils.numberToHex(tx.networkId);
                ...
                const rlpEncoded = RLP.encode([
                    ...
                    transaction.data,
                    Bytes.fromNat(transaction.networkId || '0x1'),
                    '0x',
                    '0x'
                ]);
                ...
                const signature = Account.makeSigner(Nat.toNumber(transaction.networkId || '0x1') * 2 + 35)(
        ...
        // Resolve immediately if nonce, networkId and price are provided
        if (tx.nonce !== undefined && tx.networkId !== undefined && tx.gasPrice !== undefined) {
            return Promise.resolve(signed(tx));
        }

        // Otherwise, get the missing info from the YOUChain Node
        return Promise.all([
            isNot(tx.networkId) ? _this.getId() : tx.networkId,
            ...
            if (isNot(args[0]) || isNot(args[1]) || isNot(args[2])) {
                throw new Error(
                    `One of the values 'networkId', 'gasPrice', or 'nonce' couldn't be fetched: ${JSON.stringify(args)}`
                );
            }

            return signed(extend(tx, {networkId: args[0], gasPrice: args[1], nonce: args[2]}));
        });
    }
    ...
    hashMessage(data) {
        ...
        const preamble = `\u0019YOUChain Signed Message:\n${message.length}`;
     ...
     * Decrypts account
     *
     * Note: Taken from https://github.com/youchainhqjs/ethereumjs-wallet
     *
     * @method decrypt
     *
...
// http://youchainjs.readthedocs.io/en/1.0/youchain-you-accounts.html
...
class Wallet {
    constructor(accounts) {
        ...
        this.length = 0;
        this.defaultKeyName = 'youchainjs_wallet';
    }
    ...
    save(password, keyName) {
        try {
            ...
            // we still want to support using youchain in a iframe
