...
export default class AbiCoder {
    ...
    encodeParameters(types, params) {
        return this.abiCoder.encode(this._mapTypes(types), params);
    }
    ...
    decodeParameters(outputs, bytes) {
        ...
        const res = this.abiCoder.decode(this._mapTypes(outputs), `0x${bytes.replace(/0x/i, '')}`);
